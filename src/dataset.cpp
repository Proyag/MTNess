...
std::unique_ptr<SentencePieceProcessor> load_vocab(const string& spm_path) {
  ...
  std::cerr << "Loading SentencePiece model from " << spm_path << std::endl;
  const auto spm_load_status = spm_processor->Load(spm_path);
  // Check spm load status
  if (!spm_load_status.ok()) {
    std::cerr << "SentencePiece Error: " << spm_load_status.ToString() << std::endl;
    std::abort();
  }
